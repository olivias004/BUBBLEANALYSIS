...
def append_to_csv(data, filename):
    ...
    with open(filename, mode='a', newline='') as file:
        writer = csv.DictWriter(file, fieldnames=['x_centre', 'y_centre', 'z_centre', 'dist',
         'min_density', 'min_x', 'min_y', 'min_z', 'major_axis_deg', 
         'minor_axis_deg','num_particles', 'sum_mass', 'volume', 'surface_area'])
...
def reorder_csv(filename):
    ...
    with open(filename, mode='w', newline='') as file:
        fieldnames = ['x_centre', 'y_centre', 'z_centre', 'dist',
         'min_density', 'min_x', 'min_y', 'min_z', 'major_axis_deg', 
         'minor_axis_deg','num_particles', 'sum_mass', 'volume', 'surface_area']
...
for el in rawdata_files:
	...
	csv_path = data_path + '/Sphere/Summaries/' + year + '.csv'
	...
	#PLOTS- only need to do it in this script!
	...
	plt.savefig(data_path + '/images/gas_density_inverted' + year + '.png', bbox_inches = 'tight')
	plt.close()
	...
	ra_degrees = tbl[:,9]
	dec_degrees = tbl[:,10]
	major_axis_deg = tbl[:,5]
	minor_axis_deg = tbl[:,6]

	for ra, dec, w, h in zip(ra_degrees, dec_degrees,
	        major_axis_deg, minor_axis_deg):
	    ellipse = Ellipse(xy=(ra, dec), width=w, height=h)
	    ax.add_patch(ellipse)
	plt.axis('scaled')
	plt.scatter(ra_degrees, dec_degrees, color = 'k', marker = 'x')

	plt.savefig(data_path + '/images/VOIDS_on_galaxy' + year + '.png', bbox_inches = 'tight')
	plt.close()
	...
	for el in range(0,len(tbl),1):
		...
		mask = pynbody.filt.Ellipsoid(a, b, c, cen = (x_centre, y_centre, z_centre))
		void = s.gas[mask]
		...
		SA = 4*np.pi*((((a*b)**1.6075)+((a*c)**1.6075)+((b*c)**1.6075))/(3))**(1/1.6075)

		kwargs = {
		'x_centre': x_centre,
		'y_centre': y_centre,
		'z_centre': z_centre,
		'dist': math.sqrt((x_centre)**2 + (y_centre)**2 + (z_centre)**2),
		'min_density': minimum,
		'min_x': min_idx[0],
		'min_y': min_idx[1],
		'min_z': min_idx[2],
		...
		plt.savefig(data_path + '/Sphere/scatterplots/' + year  + '/' + str(el) + '.png')
		plt.close()
		...
		for number in x:
			...
			profiling_mask = pynbody.filt.Ellipsoid(major_axis, minor_axis,
		...
		plt.savefig(data_path + '/Sphere/radial_density/'+ year+'/' +str(el)+'.png')
		plt.close()
		y.clear()
